...
class MassProfile:
    ...
    def __init__(self, galaxy1, snap, galaxy2):
        ...
        self.data = np.concatenate([self.data1 ,self.data2])
    ...
    def massEnclosed(self, ptype, radii):
        ...
        com_pos = com.COM_P(0.1) + com2.COM_P(0.1)
...
def mEncPlot(galaxy1,snap1,galaxy2,plot_name,a):
    ...
    ax.set(xlabel='r [kpc]', ylabel='$M_{enc}$ $[M_\\odot]$', 
           yscale='log', ylim=[1e8, 5e11], title=galaxy1+galaxy2+' Mass Profile')
    ax.legend()
    # save as image
    #plt.savefig(plot_name)
    plt.figure()
    plt.plot(r_arr,abs(mProf.hernquistMass(r_arr, a, M_halo_tot) - m_tot)/1e4,label='residual')
    plt.yscale('log')
    plt.ylabel('difference in hernquist and Mtotal')
    plt.xlabel('r [kpc')
    plt.title('Residuals')
...
def vCircPlot(galaxy1, snap,galaxy2, plot_name, a):
    '''
    ...
    # read in galaxy information
    mProf = MassProfile(galaxy1, snap,galaxy2) # mass profile
    ...
    ax.set(xlabel='r [kpc]', ylabel='$v_{circ}$ [km/s]', 
           title=galaxy1+galaxy2+' Rotation Curve')
...
if __name__ == '__main__':
    ...
    # plot mass profiles
    mEncPlot('MW', 455,'M31', 'MW_M31_mass.png', 1300.)
